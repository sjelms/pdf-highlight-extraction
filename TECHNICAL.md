...
├── export_json.py                     # Generate JSON exports the "raw" data of the ├── annotations.py                     # Handles the extraction of highlight annotations from PDF files.
...
- If the annotation includes a user comment (the `Text` field in the JSON annotation), add a blank line after the metadata block and then:
  ...
  > Comment text
  ```
- Multi-line comments are now correctly formatted, with each line prefixed by `>`. 
